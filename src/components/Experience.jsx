  ...
  return (
    <section id="experience" className="py-20 sm:py-28 bg-dark-800/30">
      <div className="max-w-6xl mx-auto px-5 sm:px-6" ref={ref}>
        <SectionTitle tag={tr.experience.tag} label={tr.experience.title} />
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3 sm:gap-4 mt-3 mb-4">
                    <div className="flex items-center gap-1.5 text-slate-500 text-xs"><Calendar size={11} />{item.period}</div>
                    <div className="flex items-center gap-1.5 text-slate-600 text-xs"><Clock size={11} />{item.duration}</div>
                    <div className="flex items-center gap-1.5 text-slate-600 text-xs"><MapPin size={11} />{item.location}</div>
                  </div>
                  <p className="text-slate-400 text-xs sm:text-sm leading-relaxed mb-4">{item.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {item.skills.map((s) => (
                      <span key={s} className="text-xs bg-dark-600 text-slate-400 border border-white/5 px-3 py-1 rounded-full">{s}</span>
                    ))}
                  </div>
                </div>
              </div>
