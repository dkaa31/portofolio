...
export default function Contact() {
  ...
  const socialLinks = [
    ...
    { key: 'email',     icon: <Mail size={18} />,          label: 'Email',     url: `mailto:${profile.email}`,          username: profile.email,     color: 'hover:border-accent/40 hover:text-accent' },
  ];
  ...
  return (
    <section id="contact" className="py-20 sm:py-28 relative">
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-6" ref={ref}>
        <SectionTitle tag={c.tag} label={c.title} />
        ...
        <div className="mt-10 sm:mt-14 grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
          ...
          <div className="bg-gradient-to-br from-accent/10 to-accent-blue/5 border border-accent/20 rounded-3xl p-6 sm:p-8 flex flex-col gap-4 sm:gap-5">
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-white mb-1">{c.form_title}</h3>
              <p className="text-slate-500 text-xs sm:text-sm">{c.form_sub}</p>
            </div>
            <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-slate-500">{c.name}</label>
                  <input type="text" name="name" value={form.name} onChange={handleChange} placeholder={c.name_ph} required
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-slate-500">{c.email}</label>
                  <input type="email" name="email" value={form.email} onChange={handleChange} placeholder={c.email_ph} required
                    className="bg-dark-700/80 border border-white/10 focus:border-accent/50 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 outline-none transition-colors" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-slate-500">{c.message}</label>
                <textarea name="message" value={form.message} onChange={handleChange} placeholder={c.message_ph} required rows={4}
          ...
          <div className="flex flex-col gap-4 sm:gap-6">
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-white mb-1">{c.social_title}</h3>
              <p className="text-slate-500 text-xs sm:text-sm">{c.social_sub}</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {socialLinks.map((s) => (
                <a key={s.key} href={s.url} target={s.key === 'email' ? '_self' : '_blank'} rel="noopener noreferrer"
