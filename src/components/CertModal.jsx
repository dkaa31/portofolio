...
export default function CertModal({ cert, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-dark-800 border border-white/10 rounded-t-3xl sm:rounded-3xl w-full sm:max-w-3xl shadow-2xl shadow-black/50 overflow-hidden animate-slide-up"
      ...
      >
          ...
          {cert.file ? (
            <>
              {isLocalhost && (
                <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-dark-700/95 border border-white/10 text-slate-400 text-xs px-3 sm:px-4 py-2 rounded-full shadow-lg whitespace-nowrap">
                  ...
                  Preview aktif setelah web di-deploy online
                </div>
              )}
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center bg-dark-900 z-10">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs text-slate-500">Memuat sertifikat...</p>
                  </div>
                </div>
              )}
              {isPdf ? (
                <iframe
                  src={getPreviewUrl(cert.file)}
                  title={cert.title}
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4">
              ...
              <div className="text-center px-4">
                <p className="text-sm text-slate-500 font-medium">File sertifikat belum tersedia</p>
                <p className="text-xs text-slate-700 mt-1">Akan segera ditambahkan</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-3 p-4 sm:p-6">
          <div className="text-xs text-slate-600 truncate">
            {cert.credentialId && (
              <span>ID: <span className="font-mono text-slate-500">{cert.credentialId}</span></span>
            )}
          </div>
          <div className="flex gap-2 sm:gap-3 flex-shrink-0">
            {cert.file && (
              <button
                onClick={() => downloadFile(cert.file, cert.file.split('/').pop())}
