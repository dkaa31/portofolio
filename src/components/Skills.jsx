  ...
  return (
    <section id="skills" className="py-20 sm:py-28 relative">
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-6" ref={ref}>
        <SectionTitle tag={s.tag} label={s.title} />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 mt-10 sm:mt-14">
          {skills.map((skill) => {
            ...
            return (
              <div key={skill.name} className={`glow-card group bg-gradient-to-br ${meta.color} border ${meta.border} rounded-2xl p-4 sm:p-5 hover:-translate-y-1 transition-all duration-300`}>
                ...
                <p className="text-xs text-slate-600 mt-1.5 sm:mt-2">
                  {skill.level >= 75 ? s.level_expert : skill.level >= 55 ? s.level_mid : s.level_basic}
                </p>
              </div>
            );
          })}
