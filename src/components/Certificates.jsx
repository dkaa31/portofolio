                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-3 sm:gap-4 mt-2 sm:mt-3 mb-3 sm:mb-4">
                        <div className="flex items-center gap-1.5 text-slate-500 text-xs"><Calendar size={11} />{c.issued} {cert.year}</div>
                        {cert.expiry && <div className="flex items-center gap-1.5 text-slate-600 text-xs"><Calendar size={11} />{c.valid} {cert.expiry}</div>}
                        {cert.credentialId && (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        {cert.tags && (
                          <div className="flex flex-wrap gap-2">
                            {cert.tags.map((tag) => (
                              <span key={tag} className="text-xs bg-accent/10 text-accent/80 border border-accent/10 px-2.5 py-1 rounded-full font-mono">{tag}</span>
                            ))}
                          </div>
                        )}
                        <button
